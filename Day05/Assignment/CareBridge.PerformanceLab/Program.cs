...
while (true)
{
    ...
    Console.WriteLine("7. Revenue at Risk");
    Console.WriteLine("8. with out As split query Demo");
    Console.WriteLine("9. As split query Demo");
...
static void RevenueAtRisk()
{
    ...
    var result = db.Claims.GroupBy(c => c.Status).Select(g => new
    {
        Status = g.Key,
        ClaimCount = g.Count(),
        TotalBilled = g.Sum(x => x.BilledAmount),
        TotalReiumbursed = g.Sum(x => x.ReimbursedAmt),
        ...
        .AsNoTracking()
        .ToList();
    var revenueAtRist = db.Claims.Where(c => c.Status != "Paid").Sum(c => c.BilledAmount).
      ToString();
    ...
    {
        Console.WriteLine($"{claim.Status}       {claim.ClaimCount}     {claim.TotalBilled}       {claim.TotalReiumbursed}        {claim.Gap}");
    }
    Console.WriteLine("------------------------------------------------------------");
    
    Console.WriteLine($"REVENUE AT RISK (not Paid) : {revenueAtRist} ");
...
static void AsSplitQueryDemo()
{
    using var db = new CareBridgeContext();

    Console.WriteLine();
    Console.WriteLine("As Split Query DEMO");
    Console.WriteLine("----------------------------");
    ...
    Console.WriteLine($"Encounters Loaded : {encounterCount}");
    Console.WriteLine($"Claims Loaded     : {claimCount}");
    Console.WriteLine($"Diagnoses Loaded     : {DiagnosisCount}");
...
static void WithOutAsSplitQueryDemo()
{
    using var db = new CareBridgeContext();

    Console.WriteLine();
    Console.WriteLine("With out As Split Query DEMO");
    Console.WriteLine("----------------------------");
